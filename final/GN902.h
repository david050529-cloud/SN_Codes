...
//
// 本文件收纳了以下文件的全部类型/类声明与常量(逐字保留), 配合 GN902.cpp
// (收纳全部函数实现) 即可单独编译, 无需再引用下列原文件:
...
//
// 目录自包含: 本模块(4 个顶层源文件 + Arithmetic/ + publicFunctionDoa/)可整体拷贝。
// 依赖引用(相对本文件所在目录):
//   - Arithmetic/arithmetic.h                      (ArithmeticDoa 基类 + InterferInfo)
//       └ ../publicFunctionDoa/publicFunctionDoa.h  (PublicSpace 工具函数, 位于同级目录)
//           └ Eigen3 头文件库 —— 需在编译时通过 -I 指定 Eigen 根目录(内含 Eigen/Dense);
//             如仓库根目录的 eigen-3.4.0, 不随本目录携带。
// =============================================================================
...
// ---- 外部依赖(目录自包含): ArithmeticDoa 基类 / InterferInfo / PublicSpace ----
