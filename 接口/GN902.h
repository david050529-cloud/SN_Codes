...
public:
     ...
     * @note 天线对约定:
     *       - 校正刀传 (7,7): 同一根天线接两个端口, 测通道固有相差, 每轮必须传一次;
     *       - 六测向刀传 (1,2)..(1,7): 参考天线(通道1)固定为天线 1, 通道2 循环切换;
     *       - 天线对 (8,9)/(9,8) 为固定基线采集(仅采集相位差, 不检测/测向)。
     *       轮边界以"六测向刀到齐"为准: cut 1..6 各到一帧即触发检测+测向。
     *       校正刀(cut=0)只刷新内部校正数据, 不再承担轮边界职责(设备上校正刀
     *       几十轮才来一次, 以它为边界会让检测/测向几乎不执行); 仅当切刀顺序
     *       异常导致六刀凑不齐时, 才退回"校正刀再次出现即结束上一轮"兜底。
     */
